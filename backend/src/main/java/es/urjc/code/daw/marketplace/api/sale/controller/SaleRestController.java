...
public class SaleRestController {
    ...
    public ResponseEntity<?> findCurrentDiscount(@PathVariable("type") String saleType) {
        ...
        if(!loggedUser.isAdmin() || saleType.isEmpty()) return ResponseEntity.badRequest().build();
    ...
    public ResponseEntity<UpdateSaleResponseDto> updateCurrentDiscount(@PathVariable("type") String saleType,
                                                                       @RequestBody UpdateSaleRequestDto request) {
        User loggedUser = authenticationService.getTokenUser();
        // Ensure the user that requests to update the current one time discount is an admin
        if(!loggedUser.isAdmin() || saleType.isEmpty()) return new ResponseEntity<>(HttpStatus.UNAUTHORIZED);
